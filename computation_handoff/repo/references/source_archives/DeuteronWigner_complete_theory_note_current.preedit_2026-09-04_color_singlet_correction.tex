...
\begin{equation}
 ...
 \statuscode{M2_EXPLORATORY_STATE_TO_CURRENT_REPRESENTATION_OBSTRUCTION_DEMONSTRATED},\\
...
\texttt{sys.path} user-site injection is used. This establishes a clean
fixture-validation environment, not a route for injecting the M2 Hamiltonian
into those APIs.
...
finite-K passage would require source-qualified target-composition maps and a
finite-K current,
\begin{equation}
 C_{i/f}:\mathbb C^3_{\rm spin\text{-}1}\longrightarrow\mathcal H_{M2,K9},
 \qquad J_{K9}^\mu:\mathcal H_{M2,K9}\longrightarrow\mathcal H_{M2,K9},
 \qquad J_{\rm target}^\mu=C_f^\dagger J_{K9}^\mu C_i.
 \label{eq:m2-target-current-map}
\end{equation}
Neither map is source-qualified in the current repository. In particular, the
entire six-dimensional invariant range cannot be identified isometrically
with a three-dimensional spin-one target space
($\operatorname{rank}(\mathbb C^6\to\mathbb C^3)\leq3<6$). A possible
three-dimensional image of $C_i$ would require, rather than determine, the
missing color/helicity, target-spin, and normalization composition. It is not
lawful to choose it from the numerical eigensolver.
...
$(1/6)\operatorname{Tr}(P_{K9}J_{K9}^\mu P_{K9})$ is evaluated: both still
need $J_{K9}^\mu$, and the former also needs source-defined initial/final
projectors. An arbitrary unitary rotation of the six solver vectors leaves
$P_{K9}$, its support facts, and this obstruction unchanged; no individual
vector is selected and no light-front or LPS adapter is called.

The minimal next construction is a source-qualified finite-K spin-one,
color-singlet target-composition map with initial/final momentum transfer,
charge/flavor, Fock-sector, orbital/parity, and external-normalization labels,
followed by a complete finite-K electromagnetic/current operator. The absent
information remains absent rather than zero. Consequently no current,
form factor, response, fit, or activation follows from the exploratory
projector.
...
\begin{enumerate}[label=\textbf{Step \arabic*:}]
 ...
 \item retain the accepted K9 sixfold q-sector invariant projector and derive the source-qualified finite-K spin-one/color-singlet target-composition map, including initial/final transfer, charge/flavor, Fock, orbital/parity, and normalization ownership;
 \item complete the corresponding finite-K current operator before using only the lawful projector-level transition or trace diagnostics; then compare observable finite differences and report a diagnostic sensitivity spectrum;
...
The microscopic branch has progressed beyond symbolic aspiration: it contains a finite-basis light-front architecture, source-side renormalization structures, matching and current interfaces, conditional quantum/state/observable backends, a verified diagnostic eigensolver and derivative layer, six mass-direction actions, the first retained C117 source shape, and an accepted exploratory M2 H0/K9 sixfold invariant projector.  The state-to-current audit now proves that this projector is not yet a spin-one target-current input. The next step is sharply defined: derive the finite-K spin-one/color-singlet composition map and complete finite-K current operator, alongside remaining normalization ownership, before any end-to-end C396/C117 observable response. Exploratory parameters may expose unresolved factors; they must not be relabeled as physical matching coefficients.

\begin{claimbox}{Current scientific conclusion}
The correlator-level and phenomenological theory is working. The microscopic program has an accepted diagnostic-integrity foundation, six C396 mass-direction actions, the first C117 retained source shape, and a parameter-explicit exploratory K9 invariant projector. Its state-to-current representation audit proves that no lawful spin-one-current map presently exists: neither a finite-K target-composition map nor a complete finite-K current operator is supplied. The next scientific result should construct those two source-qualified objects; physical rank, fitting, current selection, resolution convergence, and activation remain downstream.
\end{claimbox}
