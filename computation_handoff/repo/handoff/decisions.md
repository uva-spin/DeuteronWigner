...
- Files/tests: `handoff/README.md`, `handoff/ROADMAP.md`,
...
- Files/tests: `handoff/README.md`, `handoff/project_context.md`,
...
- Files/tests: `references/model_construction_note.tex`,
  ...
  `output/pdf/model_construction_note.pdf`, `handoff/README.md`.
...
- Files/tests: `references/model_construction_note.tex`, Section 15;
  `handoff/ROADMAP.md`, WP13; `handoff/README.md`; compiled and visually audited
...
# D-125: C157 numerical matching gates remain evidence-bound and fail-closed

- Date: 2026-08-19
- Status: governing for C157 and accepted locally
- Question: Does C156's executable fixed-regulator grid permit numerical
  common-IR/remainder admission when C153 contains only symbolic records?
- Decision: No. C157 publishes an explicit source/evaluator-hashed numerical
  evidence schema and gate interfaces, but accepts no point without
  authenticated residual samples. Missing common-IR and perturbative-
  remainder authority therefore yields rejected gates and empty windows.
- Justification: symbolic cancellation is not a numerical residual, and an
  arbitrary scale bracket or zero remainder would convert an unavailable
  calculation into an apparent matching result. The boundary also keeps
  C156 thresholds frozen and prevents physical-scale, running, inverse-
  matching, Q0, or TMD activation.
- Classification: scientific acceptance and provenance contract.
- Files/tests: `src/deuteron_wigner/bridge/hqcdmatchir2/core.py`,
  `data/runtime/c157_hqcdmatchir2/manifest.json`, C157 records under
  `docs/next_level/`, `tests/test_c157_hqcdmatchir2.py`; 47 C15x bridge tests
  pass.
- Revision trigger: an authenticated C157 numerical evaluator with complete
  source/evaluator provenance, route stability, and independent holdout
  residuals.
# D-126: C161 target matching remains source/evaluator gated

- Date: 2026-08-20
- Status: governing for C161 and accepted locally
- Question: Does the corrected C160 regression closure authorize numerical
  target coefficients or common-IR matching?
- Decision: No. C161 may inventory target programs and validate an explicit
  source/evaluator admission record, but it must reject numerical target
  values until authenticated source/evaluator hashes, a safe executable
  program, explicit kinematics/flavor context, and independent holdout
  samples are present.
- Evidence: C153's six primary-source rows all declare
  `numeric_project_input: false`; C158 supplies only finite-basis values;
  C160's positive gate is corrective-test authority only. C161 therefore
  reports 25 descriptor rows, zero target evaluators, zero target values,
  zero common-IR differences, zero remainders, and zero scale brackets.
- Classification: scientific acceptance and provenance contract.
- Files/tests: `src/deuteron_wigner/bridge/hqcdmatchir4/`, C161 records under
  `docs/next_level/`, `data/runtime/c161_hqcdmatchir4/manifest.json`, and
  `tests/test_c161_hqcdmatchir4.py`.
- Revision trigger: a complete authenticated target/evaluator capsule that
  satisfies the C161 admission schema and independent holdout gate.

# D-127: freeze the accepted Q0–Q2 conditional quantum backend

- Date: 2026-08-23
- Status: governing for the PennyLane backend track and accepted locally
- Question: Does the positive Q0/Q1/Q2 continuation authorize a physical
  quantum calculation or further in-place mutation of the backend?
- Decision: No. Q0/PLHQCD0, Q1/PLHQCDSTATE, and Q2/PLHQCDOBS are accepted and
  frozen as conditional finite-basis Hamiltonian-diagnostic infrastructure.
  Their public APIs, acceptance roots, and local completion commits are the
  authority boundary. No Q3 continuation is authorized by this decision.
- Evidence: Q0 closure audit is `Q0_CLOSED_POSITIVE` with package root
  `2848cb692ce20cf21f654107acbcf9ed1a803cdd1c968f576c8271ae27df3b9c`;
  Q1 acceptance is `Q1_PLHQCDSTATE_COMPLETE` with root
  `604c2797f4b12a5409a63643635c093c1653cf3b02ccccb04f7f22e2f0645547`;
  Q2 acceptance is `Q2_PLHQCDOBS_COMPLETE` with root
  `23ee186d0fb292b159a9acbfb9f52468f6d65b9fc13103014637034ae43394c1`.
  The common invariants are `COMPACT_INDEX_DIRECT_ORDER_V1`, basis order
  `q followed by qg`, K9/K11/K13 padded registers, `lightning.qubit`,
  `complex128`, and `shots=None`.
- Justification: Q0 supplies the operator authority; Q1 supplies source-
  compatible validation state routes and bounded ordinary-gate K9 replay; Q2
  supplies source-structured observable/measurement authority. The records
  explicitly reject physical parameter selection, physical state/spectrum
  claims, hardware, finite-shot execution, fits, TMDs, production objects,
  dense Pauli enumeration, and production dense unitaries.
- Classification: quantum-backend acceptance and provenance freeze.
- Files: `handoff/quantum_backend_q0_q2_freeze.md`, the three Q0/Q1/Q2
  acceptance JSON files, and the three dedicated worktrees.
- Revision trigger: a separately authorized continuation that consumes the
  frozen public APIs, proves a new acceptance vector, and publishes a new
  root without modifying the Q0–Q2 commits.
