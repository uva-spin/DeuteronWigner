#!/usr/bin/env python3
"""Plot the source-supported effective-cluster spin-1 TMD projections."""
...
INPUT = Path("outputs/parent_tmds/nonnucleonic_cluster_tmds.csv")
OUTPUT = Path("output/pdf/nonnucleonic_cluster_tmd_atlas.pdf")
...
def main() -> None:
    ...
    with PdfPages(OUTPUT) as pdf:
        for tmd, group in selected.groupby("tmd", sort=False):
            ...
            ax.set_title(f"Effective two-cluster non-nucleonic {tmd}")
            ...
            ax.text(
                0.99,
                0.80,
                "model sensitivity; cluster-motion kT only",
