...
Current status and execution direction are maintained in
`CURRENT_PROJECT_HANDOFF.md`. This file defines the durable scientific
objective and architecture; historical phase language below is not a command
to recreate old controller or bookkeeping workflows.
...
physical null prediction. The current completion path is governed by
`handoff/CURRENT_PROJECT_HANDOFF.md`; WP10 in `handoff/ROADMAP.md` records the
historical production-boundary program.
...
`handoff/CURRENT_PROJECT_HANDOFF.md` is the authoritative current execution
queue. It separates exploratory science, validated-model work, and physical
claims so that missing publication-grade authority does not block useful
derivation or sensitivity studies. `handoff/ROADMAP.md` remains the detailed
chronological record. Where historical stage language below suggests an
“initial,” “first,” or minimal boundary, that language describes development
history rather than the completion criterion.
