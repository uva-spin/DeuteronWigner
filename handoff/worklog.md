...
### Three-agent independent first-principles review

Three independent agents recalculated the OPE moment by analytic partial-wave
...
- Added root `AGENTS.md` as the mandatory future-session entry point.
...
- Corrected stale AGENTS/ROADMAP instructions that still requested the
...
- The user established that the fully self-consistent canonical
  ...
  upgrade.
- Updated the persistent agent contract, project context, roadmap, and
...
- Recorded the transition as WP13 in `handoff/ROADMAP.md` and as governing
  decision D-124; updated `AGENTS.md` so it survives context resets and
  developer handoffs.
