...
\hypersetup{
  ...
  pdftitle={DeuteronWigner: Complete Mathematical and Physical Description},
...
\LARGE A Complete Mathematical and Physical Description\\[0.25em]
...
The organizing object is a gauge-linked off-forward parton correlator.  Suppressing spin and operator indices, the hierarchy is
\begin{equation}
 W(x,\kT,\DeltaT)
...
\begin{equation}
 \boxed{\bDelta\neq\bT}
\end{equation}
not merely as notation but as a type-level invariant in \codepath{conventions.py} and \codepath{kinematics.py}.
...
\begin{equation}
 ...
 \label{eq:stf-def}
\end{equation}
with explicit recursive construction through rank four in \codepath{transverse_tensors.py}.  In two dimensions its contraction obeys
\begin{equation}
...
\end{equation}
This sign and normalization are centralized in \codepath{conventions.py}; they are not inferred separately by plotting or observable code.
...
\chapter{Gauge-invariant GTMD parent and its marginals}
\label{chap:gtmd}

\section{Quark correlator}

For quark flavor $q$, the unsubtracted zero-skewness GTMD correlator is written schematically as
\begin{align}
...
\end{equation}
at leading twist.  The path $\mathcal U$ contains the process-dependent staple direction and color representation.  A complete renormalized GTMD also requires ultraviolet renormalization, rapidity regulation, and a soft subtraction; the precise factorization scheme is discussed in \cref{chap:evolution} \cite{MeissnerMetzSchlegel2009,EchevarriaGTMD2016,Collins2011}.
...
\end{align}
The source object \codepath{SampledGTMD} stores a dense grid with terminal $3\times3$ target-helicity indices and implements these reductions with trapezoidal integration and interpolated forward slices.  Tests check PDF closure by two routes and Fourier normalization \cite{DWSourceAudit2026}.

\section{Local-current and energy--momentum moments}

The $x$ moments of the GPD reduction provide local operators,
\begin{align}
 \int\dd x\,H_{\Lambda'\Lambda}^{q[V]}(x,0,t)
 &\longrightarrow
 \langle p',\Lambda'|J_q^+(0)|p,\Lambda\rangle,
 \label{eq:vector-moment}\\
 \sum_{a=q,\bar q,g}\int\dd x\,xH_{\Lambda'\Lambda}^{a[V]}(x,0,t)
 &\longrightarrow
 \langle p',\Lambda'|T_a^{++}(0)|p,\Lambda\rangle.
 \label{eq:emt-moment}
\end{align}
For a spin-1 target these matrix elements contain charge, magnetic, quadrupole, and gravitational form-factor combinations.  The project treats such moments as independent closure constraints on the off-forward kernel; they do not replace the inclusive tensor structure function $b_1$.
...
\part{Complete spin-1 correlator basis}
...
\begin{equation}
 ...
 \label{eq:VAT}
\end{equation}
Together they define the $6\times6$ density in \cref{eq:quark-joint-density}.  The complete implemented leading-twist basis contains 18 functions for each positive-$x$ quark or antiquark species.  Names, ranks, target channels, and time-reversal parity follow the definite-rank spin-1 decomposition of \textcite{VanDaal2016}, with the physical-$S_{LL}$ adapter made explicit.
...
The operator inventory in \cref{tab:gluon-inventory} has 19 functions.  In a strictly two-dimensional transverse tensor representation, however, the numerical basis matrices multiplying $f_{1TT}^g$ and $h_{1TT}^{\perp g}$ are linearly dependent up to sign for the projected observation ensemble.  Consequently, the design matrix cannot separate both scalars without additional operator information.  The directly identifiable combination is
\begin{equation}
...
The calculation of \textcite{XieChenLu2026} provides explicit expressions for 13 T-even gluon TMDs in a continuous-mass spectator model.  DeuteronWigner uses it as an operator and qualitative benchmark.  Earlier production layers reproduced source hierarchies and tensor structure but did not constitute a complete numerical reproduction of the spectator model's fitted spectral functions and all source conventions.  Accordingly, T-even structural agreement is useful; non-Sivers gluon T-odd sectors remain model-envelope quantities rather than extracted predictions.
...
\end{align}
The minus sign is the $i^L=i^2=-1$ momentum-space partial-wave phase relative to the tabulated reduced $D$-wave transform.  The Clebsch--Gordan coefficients and spherical harmonics are explicit in \codepath{light_front.py}; they are not delegated to an uncontrolled convention.
...
The present in-house evolution implementation is deliberately heterogeneous: rank-zero unpolarized and helicity channels have the strongest perturbative support; linearly polarized gluons and several spin-dependent channels use first-nonzero-order or model interfaces; many tensor and T-odd sectors share a spin-independent evolution kernel only as a declared approximation.  A fully uniform, rank-aware, multi-$Q$ production evolution with all matching coefficients and correlated nonperturbative parameters remains open \cite{DWProductionBoundary2026}.
...
In the TMD region $P_{hT}\ll Q$, the rank-zero SIDIS structure associated with a deuteron channel $I$ has the radial $b_T$ representation
\begin{equation}
 F_{I,T}(x,z_h,P_{hT})
 =\sum_q e_q^2
...
\end{equation}
up to the common hard, leptonic, and evolution factors.  The source routine \codepath{sidis.rank_zero_sidis_structure} evaluates precisely this radial convolution with a flavor-resolved charge sum.  A convention-safe tensor ratio is then formed from the physical tensor difference rather than from an unqualified symbol $f_{1LL}$,
\begin{equation}
 R_{LL/U}^{\rm SIDIS}
...
\end{align}
The routine \codepath{sidis.tensor_cos2phi_sidis_structure} implements this expression without claiming the omitted common hard factors.  It is therefore a structure-function kernel, not by itself a detector-level asymmetry \cite{BacchettaMulders2000,BacchettaEtAl2007SIDIS}.
...
\end{equation}
when $M_D$ is expressed in energy units and $Q_D$ in length squared.  The charge-radius convention is
\begin{equation}
 r_D^2=-6\left.\frac{\dd G_C(Q^2)}{\dd Q^2}\right|_{Q^2=0}.
...
\begin{align}
 ...
 \label{eq:elastic-B}
\end{align}
For the source convention used by \codepath{form_factors.elastic_observables}, set
\begin{align}
...
\end{equation}
This factor is fixed by convention.  The C411 question is not whether to choose it, but whether the C410 source reduction already consumed it and where the remaining field, state, cell, and wave-packet normalization is owned \cite{DWC411Record2026}.
...
\end{equation}
Here $K$ controls longitudinal resolution, $N_{\max}$ the transverse oscillator truncation, and $b_{\rm HO}$ the oscillator momentum scale.  The authoritative typed basis object stores the oscillator scale in GeV, and the earlier physical-resolution manifest uses the key \codepath{bHO_GeV}.  Some later C396 metadata instead use the field name \codepath{bHO_GeVinv} for the same numerical values.  This note follows the basis-owner convention $b_{\rm HO}$ in GeV and records the later key as a metadata inconsistency; it is not silently reinterpreted as an inverse-length parameter.
...
\end{equation}
in the fixture's mass-squared convention.  Both are classified as \codepath{UNPROJECTED_DIAGNOSTIC_EIGENPAIR}; no physical quantum-number projector was applied \cite{DWC400S2Record2026}.
...
\midrule
\endhead
\codepath{ct_mass}&counterterm&Six K-local mass-direction apply paths are complete across K9, K11, and K13.\\
\codepath{ct_vacuum_energy}&counterterm&Nonmatrix vacuum direction excluded from the retained Hamiltonian; its lawful treatment is not a sparse term.\\
\codepath{ct_gluon_mass}, \codepath{ct_sector}&counterterms&Source primitive descriptors exist; no public K-local C396 apply implementation is bound.\\
\codepath{ct_boundary}&counterterm/interface&Boundary information is a nonmatrix interface, not presently a sparse Hamiltonian term.\\
\codepath{ct_truncation}&counterterm/interface&Truncation information is a nonmatrix discrepancy/interface, not presently a sparse Hamiltonian term.\\
\codepath{null_1--null_9}&source-null family&Symbolic right-null vectors exist; no finite-basis operator apply path is bound.\\
\codepath{c_C117_1}&C117 finite direction&C403--C410 construct source primitives and one retained aggregate per K; C411 leaves the final finite-C43 normalization/mixing unresolved.\\
\codepath{c_C117_2--c_C117_4}&C117 finite directions&Source-qualified descriptors exist, but no numerical source shapes or K-local apply paths are bound.\\
\bottomrule
\end{longtable}
...
\end{equation}
The coefficient $-1/2$ is applied once and $g_s^2$ remains factored in the C259/C260 convention.  C411 then defines a K-local adapter interface and records \cref{eq:finite-cell-pminus-to-mass-squared}.  It rejects accidental double conversion and unsupported promotion to the other three C117 directions.

C411's generic API requests a source-qualified $4\times4$ mixing matrix.  That requirement is a safe container, not yet a derivation that generic four-direction mixing is physically necessary for the first source shape.  The next calculation must trace the mode, state, cell, basis, and wave-packet normalizations from the actual C43 source definitions and determine whether the first direction maps diagonally, mixes with a derived subset, or remains parameterized.  A strict physical-claim route may continue to require a fully sourced record; a separate explicitly exploratory adapter may expose unresolved factors without mislabeling them as physical matching coefficients \cite{DWC401C410Record2026,DWC411Record2026,DWCurrentHandoff2026}.

\section{Historical C399 conclusion and its corrected interpretation}

C399 audited the repository, authorized capsule locations, and selected primary-source BLFQ/LFQCD routes.  It correctly established the narrow historical statement
\begin{equation}
...
\end{lstlisting}
returned 26 passed tests and zero failures.  Five warnings are inherited invalid-escape warnings in historical \codepath{lf_current.py} docstrings and do not alter the numerical results \cite{DWC400S2Record2026}.
...
\begin{enumerate}[label=\textbf{Step \arabic*:}]
 ...
 \item combine it with the six mass-direction actions and evaluate the smallest K9 diagnostic state-to-current/observable response;
 \item compare matrix, Hellmann--Feynman, and observable finite-difference derivatives and report a diagnostic sensitivity spectrum;
...
The microscopic branch has progressed beyond symbolic aspiration: it contains a finite-basis light-front architecture, source-side renormalization structures, matching and current interfaces, conditional quantum/state/observable backends, a verified diagnostic eigensolver and derivative layer, six mass-direction actions, and the first retained C117 source shape.  The next step is sharply defined: derive the remaining normalization ownership and build the first small end-to-end C396/C117 state-to-observable response.  Exploratory parameters may expose unresolved factors; they must not be relabeled as physical matching coefficients.

\begin{claimbox}{Current scientific conclusion}
The correlator-level and phenomenological theory is working.  The microscopic program has an accepted diagnostic-integrity foundation, six C396 mass-direction actions, and the first C117 retained source shapes, but no complete C117 coordinate action or physical forward map.  The next scientific result should be an explicitly scoped K9 response calculation.  Physical rank, fitting, current selection, resolution convergence, and activation remain downstream of a source-qualified version of that construction.
\end{claimbox}
...
Oscillator scale&$b_{\rm HO}$ is treated as the basis-owner momentum scale in GeV; later \codepath{bHO_GeVinv} metadata are recorded as inconsistent naming.\\
...
\midrule
LF and transverse conventions&\codepath{conventions.py}, \codepath{kinematics.py}&Typed-coordinate and transform-sign tests.\\
Spin-1 density matrix&\codepath{spin.py}&Hermiticity, trace, basis reconstruction, eigenvalue positivity.\\
GTMD parent and reductions&\codepath{gtmd.py}, \codepath{gtmd_sampling.py}&TMD/GPD/PDF/Wigner marginal closure.\\
Quark correlator&\codepath{quark_correlator.py}&Complete projection round trips, origin behavior, link reversal, minimum eigenvalue.\\
Gluon correlator&\codepath{gluon_correlator.py}&Projection reconstruction, 19-to-18 identifiability audit, positivity.\\
Definite transverse rank&\codepath{formal/transverse_rank.py}, \codepath{fourier.py}&Rank-specific Hankel transform and inverse tests.\\
Light-front variables and spin&\codepath{light_front.py}&Jacobian, Melosh rest limit, normalization and overlap checks.\\
Wave-function providers&\codepath{wavefunctions/av18.py}, \codepath{wavefunctions/cd_bonn.py}, \codepath{wavefunctions/norfolk.py}&Source parser, normalization, interpolation, and model-switch tests.\\
Nuclear convolution&\codepath{gtmd_convolution.py}, \codepath{resolved_nuclear_parent.py}&Constituent/flavor/mechanism bookkeeping and marginal closure.\\
Nuclear mechanisms&\codepath{nuclear_mechanisms.py}, \codepath{diffractive_shadowing.py}, \codepath{pion_exchange.py}, \codepath{hidden_color.py}&Exact-zero switches, source hashes, uncertainty classification.\\
TMD parent model&\codepath{parent_quark_tmd.py}, \codepath{complete_tmd_model.py}, \codepath{correlator_tmd_model.py}&Complete 18-function reconstruction and member-level positivity.\\
TMD evolution&\codepath{tmd_scheme.py}, \codepath{tmd_evolution.py}, matching modules&Scale-path, small-$b$ status, and transform checks; channel completeness remains conditional.\\
SIDIS kernels&\codepath{sidis.py}&Radial integral, Cartesian $\cos2\phi$ integral, realness and input-domain tests.\\
Elastic benchmark&\codepath{form_factors.py}&AV18 table parsing, interpolation, and $A,B,t_{20}$ recomputation.\\
Historical LF current&\codepath{lf_current.py}&Form-factor/current round trip, angular condition, and prescription spread.\\
Historical covariant/two-body current&\codepath{covariant_current.py}, \codepath{two_body_current.py}&Symmetry/source benchmarks; production selection remains open.\\
Canonical C400 current layer&\codepath{bridge/c400_s2_corrective/current_adapter.py}, \codepath{current_compare.py}&Kinematic consistency, route-local validation, GeV/MeV/$\fm^{-1}$ invariance, nonfinite rejection, canonical $G_C/G_M/G_Q$ comparison.\\
C144 derivative integrity&\codepath{bridge/c400_s2_corrective/derivative_integrity.py}&33 matrix-derivative rows, finite-difference comparison, six historical mismatch records, step/tolerance scan.\\
Diagnostic state identity&\codepath{bridge/c400_s2_corrective/state_identity.py}&Projector membership, invariance, full-space residual, Ritz/eigenpair status separation.\\
State tracking&\codepath{bridge/c400_s2_corrective/tracking.py}&Sector-partitioned assignment, rectangular sets, phase transport, Procrustes alignment, principal angles, adversarial tests.\\
C396 binding frontier&\codepath{bridge/c400_s2_corrective/coordinate_bindings.py}, C401 bridge modules&57 symbolic K-local rows and six complete mass-direction actions; no C144 proxy or physical zero completion.\\
C117 first direction&\codepath{bridge/c403_*} through \codepath{bridge/c410_*}, then \codepath{bridge/c411_c117_i2_finite_c43_adapter/core.py}&Twelve source products, three retained aggregate shapes, exact factor ownership checks, and zero complete coordinate actions pending normalization/mixing.\\
Semantic replay&\codepath{bridge/c400_s2_corrective/replay_integrity.py}&Eigenvalues, residuals, overlaps, and projector invariants instead of raw vector hashes.\\
Joint positivity&\codepath{joint_positivity.py}&Member-wise full-matrix minimum eigenvalues; projection-only refusal.\\
Controlled limits&\codepath{controlled_limits.py}&Free-constituent, pure-S, Melosh-rest, and zero-mechanism full-basis tests.\\
