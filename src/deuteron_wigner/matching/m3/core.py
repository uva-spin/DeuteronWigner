...
def operator_classification() -> dict[str, object]:
    ...
    for i in range(540):
        ...
        status = "M3_FULLY_QUALIFIED" if matched and evolved else ("M3_TMD_EVOLUTION_ONLY" if matched else "M3_UNAVAILABLE")
        reason = None if status == "M3_FULLY_QUALIFIED" else ("COLLINEAR_OR_THRESHOLD_BLOCK_UNAVAILABLE" if matched else "C20_MATCHING_UNAVAILABLE")
...
def readiness_report() -> dict[str, object]:
    return {"issued": ["C22_ENDPOINT_DISTRIBUTION_ALGEBRA_VALIDATED", "C22_PRIMARY_TWIST2_COEFFICIENT_LIBRARY_SOURCE_AUDITED_AT_DECLARED_IMPLEMENTED_ORDER", "C22_GAMMA5_SCHEME_CONVERSION_VALIDATED", "C22_COLLINEAR_NONSINGLET_SINGLET_MIXING_VALIDATED", "C22_ROUTE_A_ROUTE_B_RG_CONSISTENCY_VALIDATED", "C22_RANK_AWARE_M3_MULTIQ_CAPABILITY_VALIDATED", "C22_RESOLVED_NUCLEAR_SMALLB_OPE_VALIDATED", "C22_SCHEME_QUALIFIED_TWIST2_TMD_ENSEMBLE_VALIDATION_ONLY"], "not_issued": ["PHYSICAL_TMD_EXTRACTION", "ALL_TMD_SMALLB_COEFFICIENTS_KNOWN", "PHYSICAL_TODD_MATCHING_COMPLETE", "TWIST3_EVOLUTION_COMPLETE", "PHYSICAL_PRETZELOSITY_ZERO", "ALL_ORDER_OPE", "ALL_ORDER_EVOLUTION", "PROCESS_FACTORIZATION_READY", "W_PLUS_Y_READY", "GLOBAL_INFERENCE_READY", "PRODUCTION_READY"], "production_reachable": False, "process_reachable": False}
